...
def main():
    ...
    model = BertForSequenceGeneration.from_pretrained(args.bert_model, model_type=args.model_type)
    if args.fp16:
        model.half()
    model.to(device)
    ...
    if args.do_train:
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                # Here in bert, not need batch_sort
            ...
            if args.checkpoint:
                ...
                output_model_file = os.path.join(args.output_dir, args.task_name + "_bert_" + args.model_type + "_" + str(epoch) + "_pytorch_model.bin")
                if args.do_train:
                    torch.save(model_to_save.state_dict(), output_model_file)

    # Save a trained model
    model_to_save = model.module if hasattr(model, 'module') else model  # Only save the model it-self
    output_model_file = os.path.join(args.output_dir, args.task_name + "_bert_" + args.model_type + "_pytorch_model.bin")
    if args.do_train:
        torch.save(model_to_save.state_dict(), output_model_file)
    ...
    if args.checkpoint_id == -1:
        ...
        output_model_file = os.path.join(args.output_dir, args.task_name + "_bert_" + args.model_type + "_" + str(args.checkpoint_id) + "_pytorch_model.bin")

    # Load a trained model that you have fine-tuned
    model_state_dict = torch.load(output_model_file)
    model = BertForSequenceGeneration.from_pretrained(args.bert_model, state_dict=model_state_dict, model_type=args.model_type)
    model.to(device)
    ...
    if args.do_infer and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        for batch in tqdm(infer_dataloader, desc="infering"):
            ...
            with torch.no_grad():
                encoder_output, _ = model.bert(src_ids, segment_ids, src_mask, output_all_encoded_layers=True)
                ...
                memory_bank = model.dropout(memory_bank)
                enc_final = []
                for i in encoder_output:
                    enc_final.append(model.pooler(i))
                enc_final = torch.stack(enc_final, dim=0)

                # the real length of example in batch
                ...
                model.decoder.init_state(src_ids, memory_bank, enc_final)
